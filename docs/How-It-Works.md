...
`types/gamemaker.generated.d.ts` is generated from an installed runtime's `GmlSpec.xml`. It contains runtime functions, constants, global variables, structures, enums, asset types, and object instance variables. It is neither committed nor packaged. On first use, the standalone compiler finds the active runtime compatible with the `.yyp` IDE family and creates the project-local copy; later commands refresh it when the XML SHA-256 changes. Equally compatible installations require a machine-local choice through `ts2gml runtime`; the compiler never guesses between differing Beta, Stable, or LTS specifications. Never edit this generated file by hand.
